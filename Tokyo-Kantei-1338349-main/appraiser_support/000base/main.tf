...
module "base_network" {
  ...
  logging_bucket_name           = "${data.aws_caller_identity.current.account_id}-vpc-flowlogs"
  logging_bucket_prefix         = "${var.app_name_env_code}-vpc-flowlogs"
...
module "internal_zone" {
  ...
  name        = "reas-${lower(var.environment)}.local"
}