  ...
  stage {
    ...
    action {
      ...
      configuration = {
        ...
        "ExternalEntityLink": "https://${var.apigateway_execution_url}/test/land-predict",
        "NotificationArn": aws_sns_topic.development_admin.arn
      }
    }
  }
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        ...
        "ExternalEntityLink": "https://${var.apigateway_execution_url}/dev/land-predict",
        "NotificationArn": aws_sns_topic.development_admin.arn
      }
    }
    action {
      ...
      configuration = {
        "CustomData": "「開発へのリリース」ステージが発生したため、レビューが必要です。\\n デプロイメントを確認し、次のステージに向けて承認してください。", # Comment data for SNS notification
        "ExternalEntityLink": "https://${var.apigateway_execution_url}/dev/land-predict",
        "NotificationArn": aws_sns_topic.application_admin.arn
      }
    }
  }
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        ...
        "ExternalEntityLink": "https://${var.apigateway_execution_url}/ver/land-predict",
        "NotificationArn": aws_sns_topic.development_admin.arn
      }
    }
    action {
...
resource "aws_sns_topic" "development_admin" {
  name         = var.development_admin["name"]
  display_name = var.development_admin["display_name"]
}

resource "aws_sns_topic_subscription" "development_admin" {
  topic_arn = aws_sns_topic.development_admin.arn
  protocol  = "email"
  endpoint  = var.development_admin["email_address"]
}

resource "aws_sns_topic" "application_admin" {
  name         = var.application_admin["name"]
  display_name = var.application_admin["display_name"]
}

resource "aws_sns_topic_subscription" "application_admin" {
  topic_arn = aws_sns_topic.application_admin.arn
  protocol  = "email"
  endpoint  = var.application_admin["email_address"]
}
...
resource "aws_ssm_parameter" "dev" {
  name  = "/release-pipeline/dev_approval"
  type  = "String"
  value = "0"
}

resource "aws_ssm_parameter" "ver" {
  name  = "/release-pipeline/ver_approval"
  type  = "String"
  value = "0"
}
...
resource "aws_iam_role" "codepipeline_role" {
  name               = "codebuild-${var.pipeline_name}-pipeline-role"
...
resource "aws_iam_role" "codepipeline_source_role" {
  name               = "codebuild-${var.pipeline_name}-source-role"
