...
resource "aws_api_gateway_integration" "getcsvdownloadlink-get" {
  ...
  type                    = "AWS"
  uri                     = "arn:aws:apigateway:ap-northeast-1:lambda:path/2015-03-31/functions/arn:aws:lambda:ap-northeast-1:${var.aws_account_id}:function:GetCSVURL/invocations"
...
resource "aws_api_gateway_integration" "start-stepfunction-post" {
  ...
  passthrough_behavior    = "WHEN_NO_TEMPLATES"
  request_templates = {
    "application/json" = "#set( $body = $util.escapeJavaScript($input.json('$')) )\r\n{\r\n    \"input\": \"$body\",\r\n    \"stateMachineArn\": \"arn:aws:states:ap-northeast-1:${var.aws_account_id}:stateMachine:CreateCSV\"\r\n}"
  }
}
...
resource "aws_api_gateway_integration" "prospect-post" {
  ...
  passthrough_behavior    = "WHEN_NO_TEMPLATES"
  request_templates = {
    "application/json": "#set( $body = $util.escapeJavaScript($input.json('$')) )\r\n{\r\n    \"input\": \"$body\",\r\n    \"stateMachineArn\": \"arn:aws:states:ap-northeast-1:${var.aws_account_id}:stateMachine:CreateCSVProspect\"\r\n} "
  }
}
